...
<form action="CardExcelServlet.action"  method="post" name="formscan" id="formscan" >
  <table width="650" border="1" class="gridtable" <%=size > 0 ? "style='display:table;'": "style='display:none;'"%>>
    ...
    <tr>
		...
		<input type="text" id="pageSize" name="pageSize" style="width:25px" value="10"/>条</span><input type="button" value="设置" onclick="SetNum2()" width="40px">
		<a href="Group_CardServlet.action?pageNow=1">首页</a>
		<a href="Group_CardServlet.action?pageNow=<%=(Integer)session.getAttribute("pageNow")-1 %>">上一页</a>
		<a href="Group_CardServlet.action?pageNow=<%=(Integer)session.getAttribute("pageNow")+1 %>">下一页</a>
		<a href="Group_CardServlet.action?pageNow=<%=session.getAttribute("pageCount") %>">尾页</a><span>转到第</span><input type="text" name="jump" id="jump" style="width:25px" /><span>页</span><input type="button"  value="跳转" onClick="Jump2()" width="40px"/></td>
	  </tr>
  </table>
  </form>
