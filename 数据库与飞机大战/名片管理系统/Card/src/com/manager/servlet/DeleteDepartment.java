	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (fail != null) {
			for (int i = 0; i < fail.length; i++) {
				...
				c.setDepartmentId(Integer.parseInt(fail[i]));
				int o = dao.delete(c);
				if(o==0)
				{
		...
		if(all.equals("1"))
			response.sendRedirect("DepartmentSelectServlet.action?all=1&&pageNow="+ pageNow);
		else if(all.equals("0"))
		{
			response.sendRedirect("DepartmentSelectServlet.action?all=0&&pageNow="+ pageNow);
		}
	}
